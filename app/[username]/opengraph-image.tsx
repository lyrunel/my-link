...
async function getFontData(): Promise<ArrayBuffer | null> {
  ...
  try {
    ...
    if (buffer && buffer.byteLength > 0) {
      ...
      console.log("[OG Debug] Font downloaded successfully from primary CDN. Size:", buffer.byteLength);
      return cachedFontData;
    }
    ...
    try {
      ...
      if (buffer && buffer.byteLength > 0) {
        cachedFontData = buffer;
        console.log("[OG Debug] Font downloaded successfully from backup CDN. Size:", buffer.byteLength);
        return cachedFontData;
      }
...
async function getProfileData(username: string) {
  ...
  try {
    ...
    if (snapshot.empty) {
      console.log(`[OG Debug] Firestore returned empty result for username: "${username}"`);
      return null;
    }
    ...
    console.error(`[OG Debug] Firestore fetch failed for username "${username}":`, error);
    return null;
  }
}
...
export default async function Image({ params }: ImageParams) {
  ...
  if (!profile) {
    console.log(`[OG Debug] Profile for "${username}" is null. Rendering Fallback Page.`);
    ...
    return new ImageResponse(
      (
        ...
        >
          ...
          >
            <div style={{ fontSize: "96px", fontWeight: 900, color: "#0f172a", marginBottom: "24px", display: "flex" }}>
  ...
  const themeStyles = isNeon
    ...
    : {
        ...
        badgeBg: userTheme === "sunset" ? "#ffedd5" : userTheme === "emerald" ? "#ccfbf1" : "#e0e7ff",
        badgeText: userTheme === "sunset" ? "#ea580c" : userTheme === "emerald" ? "#0d9488" : "#4f46e5",
        badgeBorder: `1px solid ${userTheme === "sunset" ? "#fed7aa" : userTheme === "emerald" ? "#99f6e4" : "#c7d2fe"}`,
        avatarBorder: "4px solid white",
        domainColor: "#64748b",
      };

  const initialChar = profile.displayName ? profile.displayName.charAt(0).toUpperCase() : "?";

  console.log(`[OG Debug] Generating dynamic ImageResponse for user: "${profile.username}"`);
